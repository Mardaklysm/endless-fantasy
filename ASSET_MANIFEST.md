...
- Forests, mountain ranges, POIs, docks/bridges, roads, rivers, coastlines, and riverbanks are overlays or renderer concerns. Approved non-terrain object art now lives as individual transparent PNGs in `src/assets/world/current/objects/`; remaining `overlays/`, `pois/`, and `routes/` entries are explicitly marked placeholders until a clean approved replacement exists.
...
| world_current_objects | src/assets/world/current/objects/*.png | Approved world object overlays | Approved 256x256 transparent PNG objects from strict and relaxed packs | Required | P2 | Forests, mountains, settlement/city/harbor/castle POIs, docks, resources, props, ocean details | No | Curated from `D:\new_items`/`D:\Tools\rembg\bg_input` by `tools/world-object-curator/curate_world_objects.py` plus additive settlement-positive recovery from `D:\new_items\output_relaxed` by `curate_world_objects_relaxed.py`. Individual PNGs are canonical; no atlas-cell runtime path. | `drawWorldObjectCell`, `drawLocationIcon`, `drawPierDockTile`, `drawWorldOverlays` |
| world_current_placeholders | src/assets/world/current/overlays, pois, routes | Remaining world overlay/POI/route placeholders | 256x256 transparent PNG placeholders | Required until replaced | P2 | Missing dock/bridge/shipwreck/snow-forest/gate roles | No | Keep only where no approved object is clean enough. Do not silently fall back to old atlases or promote questionable art. | `drawWorldObjectCell`, `drawLocationIcon`, `drawPierDockTile`, `drawWorldOverlays` |
