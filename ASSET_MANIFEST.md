...
- Normal overworld terrain is rendered by `src/world/semantic/semanticMaskTerrainRenderer.ts` as a crisp mask-based background from semantic water/land/biome fields. It now fills deep ocean, shallow water, beach, grassland, sand, and ice/snow masks from individual PNG materials in `src/assets/world/current/terrain/`, described by `src/assets/world/current/world_asset_manifest.json`.
...
|---|---|---:|---:|---|---|---|---|---|---|
| world_current_selected_materials | src/assets/world/current/world_asset_manifest.json + current/terrain/*.png | World terrain fills | 37 approved 256x256 PNG materials | Required | P1 | Semantic mask terrain fills and raw/debug terrain fallback | No | Individual PNGs are canonical. The manifest maps deep ocean, shallow water, beach, grassland, sand, and snow/ice to one dominant fill each; extra approved materials are available variants but not random base spam. | `semanticMaskTerrainRenderer`, `drawWorldTile(tileId)`, tests |
