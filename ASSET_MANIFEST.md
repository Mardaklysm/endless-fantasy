...
- Normal overworld terrain is rendered by `src/world/semantic/semanticMaskTerrainRenderer.ts` as a crisp mask-based background from semantic water/land/biome fields. It fills deep ocean, shallow water, freshwater, beach, grassland, sand, and ice/snow masks from individual PNG materials described by `src/assets/world/current/world_asset_manifest.json`. Water, beach/sand, freshwater, and snow/ice currently use restored atlas v3 cell extracts in `src/assets/world/current/terrain_v1/`; the newer approved material pack remains in `src/assets/world/current/terrain/` for v2/future selection.
- The current overworld source of truth is the selected 37-material set imported from `D:\atlas\output\approved_materials`; only approved individual PNG fills are canonical. Old overworld terrain/object/dock atlases are not runtime sources.
- Forests, mountain ranges, POIs, docks/bridges, roads, rivers, coastlines, and riverbanks are overlays or renderer concerns. Premium non-terrain object art now lives as individual transparent PNGs in `src/assets/world/current/objects_premium/` and is preferred first for matching roles; the curated `src/assets/world/current/objects/` folder remains the backup/fallback pool.
- Drifting overworld clouds are a lightweight visual overlay from `src/assets/world/current/clouds/` and `src/assets/world/current/clouds/cloud_manifest.json`. The current five `cloud_base_*` masks are reused for every island with runtime theme tinting; non-empty biome-specific pools can override them later.
...
Note: the individual `assets/tiles/world/*` entries below remain fallback targets. The active generated overworld renders from the current selected-material manifest:

| Asset ID | Filename | Category | Size | Req | Priority | Used In | Tint/Recolor | Artist Notes | Replacement Target |
|---|---|---:|---:|---|---|---|---|---|---|
| world_current_selected_materials | src/assets/world/current/world_asset_manifest.json + current/terrain/*.png + current/terrain_v1/*.png | World terrain fills | 37 approved v2 materials plus restored atlas v3 v1 cells | Required | P1 | Semantic mask terrain fills and raw/debug terrain fallback | No | Individual PNGs are canonical. The manifest maps deep ocean, shallow water, freshwater, beach, grassland, sand, and snow/ice to one dominant fill each; restored v1 water/sand/snow are active, while v2 materials remain available variants but not random base spam. | `semanticMaskTerrainRenderer`, `drawWorldTile(tileId)`, tests |
| world_current_premium_objects | src/assets/world/current/objects_premium/*.png | Premium world object overlays | Approved premium transparent PNG objects, mostly 512x512 atlas cells | Required | P1 | Primary forests, mountains, settlement/city/harbor/castle POIs, docks, bridges, resources, hazards, props, ocean details | No | Imported from `D:\Tools\rembg\bg_output` by `tools/world_assets/import_premium_world_objects.py`. The source is treated as approved for this pass; the importer performs technical validation and atlas slicing only. Premium mappings are resolved before backup mappings. | `drawWorldObjectCell`, `drawLocationIcon`, `drawPierDockTile`, `drawWorldOverlays` |
| world_current_objects | src/assets/world/current/objects/*.png | Backup world object overlays | Approved 256x256 transparent PNG objects from strict and relaxed packs | Required fallback | P2 | Backup pool for object/POI/overlay roles without a premium match | No | Curated from `D:\new_items`/`D:\Tools\rembg\bg_input` by `tools/world-object-curator/curate_world_objects.py` plus additive settlement-positive recovery from `D:\new_items\output_relaxed` by `curate_world_objects_relaxed.py`. Keep this folder; do not restore user-deleted backup PNGs unless a real active reference cannot be replaced. | `drawWorldObjectCell`, `drawLocationIcon`, `drawPierDockTile`, `drawWorldOverlays` |
| world_current_clouds | src/assets/world/current/clouds/*.png + cloud_manifest.json | Overworld cloud overlay | 5 transparent high-res base cloud PNG masks | Required fallback | P3 | Slow top-band drifting flavor overlay on the overworld only | Yes, runtime theme tint | Imported from `D:\Tools\rembg\bg_output_2` by `tools/world_assets/import_world_clouds.py`. The base clouds are reused everywhere; theme tint/alpha/speed settings cover grassland, jungle, snow, desert, swamp, volcanic, and deadland moods. | `OverworldCloudOverlay`, `WORLD_CLOUD_MANIFEST` |
| world_current_generated_fallbacks | generated in `src/main.ts` | Final fallback object/POI rendering | Procedural Graphics fallback | Required safety net | P3 | Any role with no premium or backup texture available | No | No manifest placeholder PNG records are currently required. Keep generated fallback behavior for missing textures/roles, but do not restore deleted generic placeholder PNGs for runtime. | `drawLocationIcon`, generated marker/object fallback paths |
...
| tile_bridge | assets/tiles/world/bridge.png | World tile | 16x16 | Optional | P2 | Fallback water crossings | Partial | Wooden/stone bridge over water; current harbor dock/bridge visuals use explicit files in `src/assets/world/current/routes/`. | Future route/bridge overlay |
