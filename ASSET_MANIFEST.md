...
- Forests, mountain ranges, POIs, docks/bridges, roads, rivers, coastlines, and riverbanks are overlays or renderer concerns. Current non-terrain world art in `src/assets/world/current/overlays/`, `pois/`, and `routes/` is explicitly marked placeholder in the manifest until approved selected sprites exist.
...
| world_current_placeholders | src/assets/world/current/overlays, pois, routes | Temporary world overlay/POI/route sprites | 256x256 transparent PNG placeholders | Required until replaced | P2 | Forests, mountains, POIs, docks/bridges, ocean details | No | Generated placeholders exist only because the curated 37 selected assets were terrain fills. Replace with approved selected sprites; do not fall back to old atlases. | `drawWorldObjectCell`, `drawLocationIcon`, `drawPierDockTile`, `drawWorldOverlays` |
