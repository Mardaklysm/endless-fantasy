...
| world_current_objects | src/assets/world/current/objects/*.png | Approved world object overlays | 80 approved 256x256 transparent PNG objects | Required | P2 | Forests, mountains, POIs, docks, resources, props, ocean details | No | Curated from `D:\new_items` plus additive raw sources in `D:\Tools\rembg\bg_input` by `tools/world-object-curator/curate_world_objects.py`. Individual PNGs are canonical; no atlas-cell runtime path. | `drawWorldObjectCell`, `drawLocationIcon`, `drawPierDockTile`, `drawWorldOverlays` |
