...
- Normal overworld terrain is rendered by `src/world/semantic/semanticMaskTerrainRenderer.ts` as a crisp mask-based background from semantic water/land/biome fields. The fixed-grid 8x8 atlas `src/assets/world/atlas_v3.png` plus `src/assets/world/atlasV3.manifest.json` supplies the repeated texture crops for deep water, shallow water, beach, grass, sand, and snow/ice masks. Raw square atlas tiles remain available for debug/fallback, but they are not the normal terrain presentation. The current atlas has 64 usable terrain cells; empty black cells are no longer part of the active sheet.
...
|---|---|---:|---:|---|---|---|---|---|---|
| atlas_v3 | src/assets/world/atlas_v3.png + atlasV3.manifest.json | World terrain atlas | 1024x1024 sheet, 8x8 grid, 128px cells | Required | P1 | Mask-rendered terrain texture sources, raw-tile debug/fallback terrain, and semantic tile metadata | No | Normal gameplay samples canonical atlas cells as repeating textures inside semantic masks, rather than drawing the cells as a square tilemap. All 64 cells are classified as terrain. Variant art should become sparse overlays or controlled patches later, not random base replacement. POIs render from separate marker/object art. | `semanticMaskTerrainRenderer`, `drawWorldTile(tileId)`, F6 raw tiles |
