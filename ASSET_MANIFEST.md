...
- Normal overworld terrain uses the fixed-grid 8x8 atlas `src/assets/world/atlas_v3.png` plus `src/assets/world/atlasV3.manifest.json` as the base art. Normal semantic base terrain maps to one canonical tile per role: `deep_water`, `shallow_water`, `beach_sand`, `medium_grass`, `bright_sand`, and `clean_snow`. `src/world/semantic/semanticTerrainRenderer.ts` generates only a transparent edge overlay from water/land/biome masks for coast, shallow-water, beach, and biome boundary accents. The current atlas has 64 usable terrain cells; empty black cells are no longer part of the active sheet.
...
|---|---|---:|---:|---|---|---|---|---|---|
| atlas_v3 | src/assets/world/atlas_v3.png + atlasV3.manifest.json | World terrain atlas | 1024x1024 sheet, 8x8 grid, 128px cells | Required | P1 | Normal terrain, raw-tile debug/fallback terrain, and semantic tile metadata | No | Runtime uses exact 8x8 cell rectangles for normal base terrain, with one canonical tile per semantic terrain type. Semantic rendering adds only a transparent edge overlay; it does not replace terrain tiles. All 64 cells are classified as terrain. Variant art should become sparse overlays or controlled patches later, not random base replacement. POIs render from separate marker/object art. | `drawWorldTile(tileId)`, terrain cache, F6 raw tiles |
