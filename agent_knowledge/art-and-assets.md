...
- World object overlays: generated dungeons, harbors, landmarks, ocean details, forests, and mountain ranges use manifest mappings in `src/data/worldCurrentAssets.ts`. Current overlay and POI files are transparent placeholders under `src/assets/world/current/overlays/` and `src/assets/world/current/pois/`; replace them with approved selected sprites when available. Do not fall back to deleted old atlases.
...
- Active current overworld placeholders: 24 transparent 256x256 PNG overlay/POI/route placeholders in `src/assets/world/current/overlays/`, `pois/`, and `routes/`, marked `placeholder` in `world_asset_manifest.json`.
