...
- Overworld terrain support: generated worlds keep `src/assets/world/atlas_v3.png` through `src/assets/world/atlasV3.manifest.json` and `src/data/worldTiles.ts` for semantic compatibility, debug raw-tile rendering, fallback drawing, and selected helpers. The normal overworld base terrain is now a code-generated semantic texture with crisp procedural fills and boundary-only pixel overlays instead of hard square atlas cells or whole-map blur. The active atlas grid remains 8x8 with 64 classified terrain cells, including grass, beaches/coasts, shallow/deep water, forests/jungle, roads, mountains, darkland, and lava/volcanic tiles. Valid raw/fallback tile draws use `ATLAS_V3_SOURCE_INSET = 3`, cropping dirty source-cell edges while drawing the clean interior into the full destination tile.
...
- Generated overworld terrain should avoid using fixed-orientation coast/foam cells as random stamps. Normal gameplay uses semantic rendering with crisp terrain fills and narrow edge-only pixel overlays for shallow water, beach, coast foam/tint, and grass/sand/ice edges instead of direction-specific transition tile stamping or global smoothing.
...
- Do not implement runtime map-level seam blending for `atlas_v3` tile placements. Raw/fallback tile rendering and debug previews should crop valid source cells inward and must not mutate stamped atlas pixels by mixing water/grass or other neighboring terrain colors after placement. The current semantic terrain background is a separate generated texture and should stay crisp: full-cell fills first, boundary overlays only where neighboring semantic cells actually change.
