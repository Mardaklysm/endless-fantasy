...
- `tools/world_assets/import_selected_world_assets.py`: imports the approved external pack from `D:\atlas\output\approved_materials` into `src/assets/world/current/`, writes `world_asset_manifest.json`, and generates explicitly marked placeholders for missing overlay/POI/route roles.
...
- Overworld terrain support: generated worlds use `src/assets/world/current/world_asset_manifest.json` and selected individual PNG fills under `src/assets/world/current/terrain/` for semantic mask texture sources. Normal gameplay uses `src/world/semantic/semanticMaskTerrainRenderer.ts` to render deep water, shallow water, beach, grass, sand, and ice/snow as crisp semantic masks filled with approved materials. `src/data/worldTiles.ts` is now semantic compatibility metadata only; it keeps tile IDs, walkability, movement cost, encounter family, and tags without atlas source rectangles.
- Route overlays: normal gameplay uses `src/world/semantic/semanticRouteRenderer.ts` to draw semantic roads and rivers as transparent procedural stroke overlays. Roads use a warm dirt fill, darker outline, round joins/caps, deterministic jitter, and small speckles. Rivers use a banked procedural brush with shadowed edges, muted water, curved strokes, source/mouth pools, and small deterministic flecks. These are not terrain tiles and do not require giant road or river atlases.
- Harbor docks and bridges: generated harbor dock/bridge markers render from explicit placeholder route sprites under `src/assets/world/current/routes/` until approved selected route art exists.
- World object overlays: generated dungeons, harbors, landmarks, ocean details, forests, mountain ranges, resources, and props use manifest mappings in `src/data/worldCurrentAssets.ts`. Approved transparent object PNGs now live under `src/assets/world/current/objects/` and replace many placeholder roles; remaining placeholder files under `overlays/`, `pois/`, and `routes/` stay only for roles without clean approved art. Current POI sprites render with manifest scale/anchor metadata so settlements, harbors, castles, and towers sit inside their footprint as actual landmarks instead of generic centered squares. Do not fall back to deleted old atlases.
...
- Active current overworld objects: approved 256x256 transparent PNG object sprites in `src/assets/world/current/objects/`, selected from the strict `D:\new_items` pass and additive relaxed recovery under `D:\new_items\output_relaxed`. The relaxed pass adds game-ready settlement/city/harbor/castle POIs while keeping touchup-needed sprites outside runtime.
- Active current overworld placeholders: explicit transparent 256x256 PNG overlay/POI/route placeholders remain in `src/assets/world/current/overlays/`, `pois/`, and `routes/` for missing vertical dock, bridge, shipwreck, snow-forest, and dedicated gate roles, marked `placeholder` in `world_asset_manifest.json`.
...
- Active overworld material rendering starts from `world_asset_manifest.json` texture keys. There is no old 8x8 overworld atlas slicing path in normal runtime, and F6 `rawTiles` uses current individual material PNGs for debug fallback.
- Active overworld object rendering also starts from `world_asset_manifest.json` texture keys. Approved object sprites are individual transparent PNGs under `current/objects/`; the dense atlas preview from the curator is debug-only and must not become a runtime source of truth.
...
- The active overworld direction starts from semantic masks/fields: archipelago land masks, shallow-water halos, beach bands, grass/sand/ice biomes, elevation/ridges, styled river/road overlays, forest/mountain range object overlays, and POI placement. Do not return to designing a huge transition tileset as the core architecture.
- Roads, rivers, beaches/coasts, shallow water, forests/jungle, mountains, and POIs are renderer/overlay concerns. The selected 37 terrain fills are base materials only. Generated fallback art still remains for missing textures and for lightweight non-atlas overlays such as sea route dots and optional ocean details.
...
- F6 road and river graph strokes/dots are semantic debug information, not finished map art. Keep debug nodes, source/mouth markers, rejected routes, and pathfinding diagnostics out of normal gameplay. Normal gameplay should show only the styled procedural route overlay.
