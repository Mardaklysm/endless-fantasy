...
- Harbor docks and bridges: generated harbor dock/bridge markers map to existing current object PNGs where a clean approved object exists; road and river geometry remains procedural.
- World object overlays: generated dungeons, harbors, landmarks, ocean details, forests, mountain ranges, resources, and props use manifest mappings in `src/data/worldCurrentAssets.ts`. Approved transparent object PNGs live under `src/assets/world/current/objects/`; the old generic overlay/POI/route icon folders were intentionally removed and should not be restored for runtime. Current POI sprites render with manifest scale/anchor metadata so settlements, harbors, castles, and towers sit inside their footprint as actual landmarks instead of generic centered squares. Do not fall back to deleted old atlases.
