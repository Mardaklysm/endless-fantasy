...
- Overworld terrain support: generated worlds keep `src/assets/world/atlas_v3.png` through `src/assets/world/atlasV3.manifest.json` and `src/data/worldTiles.ts` for normal base terrain, semantic compatibility, debug raw-tile rendering, fallback drawing, and selected helpers. Normal semantic base terrain uses one canonical tile per role: `deep_water`, `shallow_water`, `beach_sand`, `medium_grass`, `bright_sand`, and `clean_snow`; `TERRAIN_VARIANT_MODE` is `"off"`. The semantic terrain renderer generates only a transparent boundary overlay for shallow-water, coastline, wet sand, beach/inland, and grass/sand/ice edge accents; it does not replace atlas terrain art with procedural fills. The active atlas grid remains 8x8 with 64 classified terrain cells, including grass, beaches/coasts, shallow/deep water, forests/jungle, roads, mountains, darkland, and lava/volcanic tiles. Valid raw/fallback tile draws use `ATLAS_V3_SOURCE_INSET = 3`, cropping dirty source-cell edges while drawing the clean interior into the full destination tile.
...
- Active overworld slicing starts from exact 8x8 atlas math from `atlasV3.manifest.json`, then applies the shared source inset: `sx = tile.source.x + 3`, `sy = tile.source.y + 3`, `sw = tile.source.width - 6`, `sh = tile.source.height - 6`. Normal gameplay uses this atlas art as the terrain base. Do not reintroduce classic special tiles, old 10x10 assumptions, chroma-keying, global black transparency, or debug-grid art for runtime terrain.
...
- Generated overworld terrain should avoid using fixed-orientation coast/foam cells as random stamps. Normal gameplay uses existing canonical atlas terrain cells as the base and semantic rendering only for transparent, narrow edge overlays for shallow water, beach, coast foam/tint, and grass/sand/ice boundaries. Do not return to procedural full-cell terrain replacement, random variant spam, direction-specific transition tile stamping, or global smoothing.
...
- Do not implement runtime map-level seam blending for `atlas_v3` tile placements. Raw/fallback/normal base tile rendering and debug previews should crop valid source cells inward and must not mutate stamped atlas pixels by mixing water/grass or other neighboring terrain colors after placement. The current semantic terrain renderer is a transparent overlay only: boundary accents where neighboring semantic cells actually change, never full-cell replacement fills.
