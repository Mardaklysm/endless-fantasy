...
- Route overlays: normal gameplay uses `src/world/semantic/semanticRouteRenderer.ts` to draw semantic roads and rivers as transparent procedural stroke overlays. Roads use worn terrain edging, a warm dirt body, muted highlight, round joins/caps, deterministic jitter, and small speckles. Rivers use a much wider banked procedural brush with shadowed edges, muted water, curved strokes, source/mouth pools, small deterministic flecks, and subtle mouth foam. These are not terrain tiles and do not require giant road or river atlases.
- Harbor docks and bridges: generated harbor dock/bridge markers and road-river bridge candidates map to existing current object PNGs where a clean approved object exists; road and river geometry remains procedural.
