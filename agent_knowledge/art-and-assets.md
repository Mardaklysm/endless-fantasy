...
- Overworld terrain support: generated worlds keep `src/assets/world/atlas_v3.png` through `src/assets/world/atlasV3.manifest.json` and `src/data/worldTiles.ts` for semantic mask texture sources, semantic compatibility, debug raw-tile rendering, fallback drawing, and selected helpers. Normal gameplay uses `src/world/semantic/semanticMaskTerrainRenderer.ts` to render deep water, shallow water, beach, grass, sand, and ice/snow as crisp semantic masks filled with atlas texture crops. The active atlas grid remains 8x8 with 64 classified terrain cells, including grass, beaches/coasts, shallow/deep water, forests/jungle, roads, mountains, darkland, and lava/volcanic tiles. Valid raw/fallback tile draws use `ATLAS_V3_SOURCE_INSET = 3`, cropping dirty source-cell edges while drawing the clean interior into the full destination tile.
...
- Roads, beaches/coasts, shallow water, forests/jungle, and volcanic support have real atlas cells for debug/fallback support, but normal road/river presentation is stroke-rendered from semantic paths. Generated fallback art still remains for missing textures and for lightweight non-atlas overlays such as sea route dots and optional ocean details.
