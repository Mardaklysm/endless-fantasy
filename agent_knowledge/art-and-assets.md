...
- Cloud overlay: overworld mode uses `src/world/cloudOverlay.ts` plus `src/data/worldCloudAssets.ts` to render slow, semi-transparent clouds in the top band above world visuals and below UI. Current source art is five reusable base cloud masks from `src/assets/world/current/clouds/`; jungle, snow, desert, swamp, volcanic, and deadland flavor comes from runtime tint/alpha/speed settings without affecting generation or collision.
...
- Active overworld cloud rendering starts from `current/clouds/cloud_manifest.json`. The overlay is visual-only, world-mode-only, and may be toggled with F7 for debugging; cloud texture selection and tint transitions must not consume gameplay RNG or alter semantic world generation.
