...
- World object overlays: generated dungeons, harbors, landmarks, ocean details, forests, mountain ranges, resources, and props use manifest mappings in `src/data/worldCurrentAssets.ts`. Premium transparent object PNGs live under `src/assets/world/current/objects_premium/` and are resolved first; curated transparent backup objects live under `src/assets/world/current/objects/`. The old generic overlay/POI/route icon folders were intentionally removed and should not be restored for runtime. Current POI sprites render with manifest scale/anchor metadata so settlements, harbors, castles, and towers sit inside their footprint as actual landmarks instead of generic centered squares. Mountain ranges render biome-aware rocky/snowy footprint graphics for every accepted semantic mountain cell, then place a small number of coverage-spread premium ridge sprites over the footprint; do not return to mixed rock/cliff/mountain sprite spam or single random sign icons. Do not fall back to deleted old atlases.
