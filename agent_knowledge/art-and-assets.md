...
- Overworld terrain support: generated worlds use `src/assets/world/current/world_asset_manifest.json` and selected individual PNG fills under `src/assets/world/current/terrain/` for semantic mask texture sources. Normal gameplay uses `src/world/semantic/semanticMaskTerrainRenderer.ts` to render deep water, shallow water, freshwater rivers/lakes, packed-dirt roads, beach, grass, sand, and ice/snow as crisp semantic masks filled with approved materials. `src/data/worldTiles.ts` is now semantic compatibility metadata only; it keeps tile IDs, walkability, movement cost, encounter family, and tags without atlas source rectangles.
