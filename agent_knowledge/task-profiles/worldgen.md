...
- The runtime overworld uses the semantic mask terrain cache for the polished organic roads. Doubling the map to 576x384 at 32px tiles exceeds common 16384px texture limits for a single full-world cache; do not bypass the semantic cache into `drawWorldTerrain.ts` fallback tiles, because fallback roads are full-cell rectangular blocks. Larger worlds need a chunked semantic terrain cache or equivalent renderer-safe plan first.
- Premium POI assets are not automatically safe just because they are in a preferred folder. Verify alpha/transparency before mapping them into world POIs; raw opaque-background assets will render as visible square backgrounds.
