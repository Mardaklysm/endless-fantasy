...
Styled road and river overlays use `src/world/semantic/semanticRouteRenderer.ts`, not road/river tile atlases. Runtime builds a transparent full-world canvas texture from semantic road graph paths and river paths, draws rivers first, then roads on top for temporary bridge crossings, and caches/crops that texture in `drawWorld()` between terrain and object overlays. Normal route mode uses styled strokes with outlines, muted fills, deterministic jitter, and dirt speckles. Rivers use a banked procedural brush with muted water, shadowed banks, curved strokes, source/mouth pools, and small deterministic flecks; F6 `roads`/`rivers` remain explicit debug overlays with nodes/source/mouth markers.

The active overworld art set is `src/assets/world/current/` plus `world_asset_manifest.json`. `drawWorldTile` raw/debug fallback resolves semantic tile IDs to current material texture keys. There is no normal-runtime overworld atlas crop path; removed special tilesets, generated 10x10 atlases, and old overworld sheets are not active runtime terrain.
...
Seed-derived ocean overlays, forests, mountain range cells, current-folder dock/bridge sprites, POIs, player, and UI are separate layers above the mask terrain background and styled route overlay. Road and river graph data remains in the semantic world and normal gameplay renders it through the styled stroke cache; F6 `roads` and F6 `rivers` show graph diagnostics only. `drawLocationIcon` prefers current POI texture mappings from `world_asset_manifest.json`, uses current asset scale/anchor metadata for approved object sprites, then falls back to marker textures or generated art.
