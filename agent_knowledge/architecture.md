...
- `WORLD_W = 96`
- `WORLD_H = 64`
- Seam repair uses deterministic dither-pick between clean neighbor samples — no color averaging/blending.
- `SEAM_TARGET_RADIUS = 3` — only ±3px around tile boundaries are repaired.
- `INTERIOR_SAMPLE_INSET = 4` — clean samples taken 4px inside each tile (fallback to 8px).
- `NEAR_BLACK_LUMINANCE_THRESHOLD = 38` — artifact detection.
- `CLEAN_SAMPLE_MIN_LUMINANCE = 18` — minimum luminance for valid clean sample (separate from detection).
- `RELATIVE_DARKNESS_THRESHOLD = 20` — relative darkness detection.
- The old seam pixel is ONLY a destination mask trigger, never a color source. Replacement pixels are chosen from clean interior samples via deterministic dithering.
