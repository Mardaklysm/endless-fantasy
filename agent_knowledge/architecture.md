...
Normal overworld base terrain is now generated from semantic masks/fields by `src/world/semantic/semanticTerrainRenderer.ts`. It creates a Phaser canvas texture sized to `world.width * TILE` by `world.height * TILE`, renders deep ocean, shallow-water halos, beaches, grass/sand/ice fills, coastline foam/tint, and softened biome edges, then displays that cached texture behind all overlays. Collision, encounters, POIs, roads, rivers, harbors, and island metadata still come from the semantic grid, not from the rendered image.
