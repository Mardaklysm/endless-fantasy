...
Styled road and river overlays use `src/world/semantic/semanticRouteRenderer.ts`, not road/river tile atlases. Runtime builds a transparent full-world canvas texture from semantic road graph paths and river paths, draws rivers first, then roads on top for temporary bridge crossings, and caches/crops that texture in `drawWorld()` between terrain and object overlays. Normal route mode uses layered road strokes with worn terrain edging, dirt body, muted highlight, deterministic jitter, and speckles. Rivers use a wide banked procedural brush with muted water, darker edges, curved strokes, source/mouth pools, small deterministic flecks, and mouth foam; F6 `roads`/`rivers` remain explicit debug overlays with nodes/source/mouth markers.
