...
Normal overworld base terrain is now generated from semantic masks/fields by `src/world/semantic/semanticTerrainRenderer.ts`. It creates a Phaser canvas texture sized to `world.width * TILE` by `world.height * TILE`, renders crisp full-cell deep ocean, shallow water, beach, grass, sand, and ice fills with deterministic pixel patterns, then adds narrow dithered edge strips for shallow-water, coast foam/wet sand, beach-to-inland, and grass/sand/ice biome boundaries. Collision, encounters, POIs, roads, rivers, harbors, and island metadata still come from the semantic grid, not from the rendered image.

The active `atlas_v3` tileset at `src/assets/world/atlas_v3.png` plus `src/assets/world/atlasV3.manifest.json` remains loaded for debug raw-tile rendering, fallback drawing, and selected overlay/legacy helpers. `drawWorldTile` crops valid 8x8 atlas cell rectangles inward with `ATLAS_V3_SOURCE_INSET = 3` via `atlasV3SourceRectWithInset`, then draws the clean interior into the full 32 layout pixel destination tile. All 64 atlas cells are still classified terrain IDs, but square atlas terrain is no longer the normal visible overworld style. The classic special tileset and old generated 10x10 atlas are not active runtime terrain.
...
Generated overworld terrain is cached as a Phaser canvas texture when a world seed/layout is built. The cache is produced from semantic fields, not by stamping the full source island or by drawing a giant transition atlas. It renders at final world pixel size with `imageSmoothingEnabled = false`; interiors stay crisp, while only actual terrain boundaries receive small pixelated tint/foam/dither overlays. Collision, walkability, POIs, and tile IDs remain unchanged. POI interaction footprints stay larger than their drawn overlays; `drawLocationIcon` scales the visual object inside the footprint so caves, chests, shrines, and harbors do not cover whole island chunks.
...
Roads, rivers, sea-route dots, seed-derived `world_objects` ocean overlays, forests, mountains, pier-atlas dock/bridge cells, POIs, player, and UI are separate layers above the generated terrain texture. Road visuals are drawn from cardinal road masks as thin Phaser `Graphics` strokes instead of being baked into terrain cells. `drawLocationIcon` prefers POI `objectId` values from `world_objects` for generated dungeons/landmarks/harbors, then falls back to legacy marker textures or generated art.

F6 semantic debug cycling includes normal/off, raw square tiles, semantic masks, distance bands, walkability, overlay policy, mountain candidates/accepted cells, forest soft-terrain cells, island themes, POI IDs/clearance, roads, and rivers.
