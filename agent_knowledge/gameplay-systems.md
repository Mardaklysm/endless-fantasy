...
- Movement is tile-anchor based with arrow keys/WASD: each accepted direction starts a one-tile step from the current tile center to the next tile center.
- Holding a direction continues smoothly tile-by-tile; releasing finishes the current step and stops centered on the destination tile.
