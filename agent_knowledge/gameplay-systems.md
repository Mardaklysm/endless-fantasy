...
- The world is a 64x40 tile grid generated by `makeWorld()`.
- Player position is `worldPos`.
- Movement is grid-logical but visually smooth with arrow keys/WASD.
- On directional input, the scene starts a tile step, writes interpolated coordinates into persistent visual position state, then commits the logical grid position when the step completes.
- Holding a direction keeps walking tile-to-tile; releasing stops after the current tile step.
- Shift keeps faster exploration movement, but still uses smooth tile interpolation rather than skipped tiles.
- Visual position and logical tile position are intentionally separate during a step. Do not recompute rendering from stale logical tile coordinates mid-step.
- Terrain types include plains, forest, hills, mountain, water, deep water, sand, and road.
- Mountains block movement unless skyship is unlocked.
- Water blocks movement until boat or skyship is unlocked.
...
- Encounter counter decrements only after completed tile steps; forests/hills are more dangerous.
- World encounter tables are grouped by terrain/region.
...
- Player attacks, spells, items, defend, and run attempts resolve immediately on that actor's turn.
