...
- Movement is tile-anchor based with arrow keys/WASD: each accepted direction starts a one-tile step from the current tile's bottom-center feet anchor to the next tile's bottom-center feet anchor.
- Holding a direction continues smoothly tile-by-tile; releasing finishes the current step and stops with the leader's feet anchored on the destination tile.
...
- Exploration collision checks the destination tile/feet anchor, not the full character sprite rectangle.
