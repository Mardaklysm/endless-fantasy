...
- Road semantic paths render in normal gameplay through styled procedural stroke overlays; inspect graph nodes/source/mouth diagnostics through F6 debug modes. River bodies render from semantic river masks through the current freshwater material as connected cardinal mask shapes, not full-square atlas stamps. Unbridged rivers block walking, while road-river bridge candidates remain walkable and render bridge/crossing stamps. Future river polish should add dedicated directional river edge masks or better material fills, not blue stroke/tube rendering.
...
- Semantic overworld collision and overlay rules now separate visual-only, soft-terrain, hard-block, and POI interaction policies. Forests are passable soft terrain for now, unbridged rivers and connected mountain massif masks are hard blockers, and road-river bridge candidates remain walkable. Mountain ranges use theme-aware massif seed caps/snow rules, clean up tiny fragments to zero singletons, and render as dense one-asset-per-massif clustered patches over semantic collision cells. Future polish should tune encounter effects for soft terrain rather than making forests collision walls again.
...
- Route/dock/bridge visuals are current-manifest objects. Road-river bridge candidates are semantic records that also feed the visible bridge list; keep improving those stamps through current `objects_premium/` or backup `objects/` assets, not old atlas or route placeholder folders.
