...
- Runtime overworld terrain now uses canonical `atlas_v3` terrain tiles as the base art plus a transparent semantic edge overlay for shallow-water halos, coastline tint/foam, wet sand, and grass/sand/ice boundaries. Future polish should tune the edge overlay or add minimal brush-style overlay assets, not replace the atlas with procedural fills, random variant spam, or hundreds of coastline transition tiles.
