...
- Semantic overworld collision and overlay rules now separate visual-only, soft-terrain, hard-block, and POI interaction policies. Forests are passable soft terrain for now, unbridged rivers and connected mountain massif masks are hard blockers, and road-river crossing candidates remain walkable. Mountain ranges use theme-aware massif seed caps/snow rules, clean up tiny fragments to zero singletons, render blocked cells as semantic mountain terrain in the mask renderer, and place sparse visual-only ridge sprites on top. Future polish should tune encounter effects for soft terrain rather than making forests collision walls again.
