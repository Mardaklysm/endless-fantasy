...
- Road and river semantic paths now render in normal gameplay through styled procedural stroke overlays; inspect graph nodes/source/mouth diagnostics through F6 debug modes. Rivers use a wide banked-brush pass with curved muted strokes, pools, flecks, and mouth foam. Unbridged rivers block walking, while road-river bridge candidates remain walkable and render bridge/crossing stamps. Future route polish should tune the brush further or improve crossing stamps rather than creating giant road/river atlases.
...
- Semantic overworld collision and overlay rules now separate visual-only, soft-terrain, hard-block, and POI interaction policies. Forests are passable soft terrain for now, unbridged rivers and mountain ranges are hard blockers, and road-river bridge candidates remain walkable. Mountain ranges use theme-aware caps/snow rules and render as clustered patches over semantic collision cells. Future polish should tune encounter effects for soft terrain rather than making forests collision walls again.
