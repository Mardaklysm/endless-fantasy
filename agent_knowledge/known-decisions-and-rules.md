...
- The active overworld art set is `src/assets/world/current/` plus `src/assets/world/current/world_asset_manifest.json`. It contains 37 approved individual 256x256 terrain fills imported from `D:\atlas\output\approved_materials`, the original strict approved object sprites from `D:\new_items\output\approved_objects`, and additive relaxed game-ready object sprites from `D:\new_items\output_relaxed\game_ready_objects`. User-deleted generic overlay/POI/route placeholder PNGs should stay deleted; missing roles use existing-object mappings or procedural renderer fallbacks. Old overworld terrain/object/dock atlases are not runtime sources.
