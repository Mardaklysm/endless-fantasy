...
- The active overworld art set is `src/assets/world/current/` plus `src/assets/world/current/world_asset_manifest.json`. It contains 37 approved individual 256x256 terrain fills imported from `D:\atlas\output\approved_materials`, the original strict approved object sprites from `D:\new_items\output\approved_objects`, additive relaxed game-ready object sprites from `D:\new_items\output_relaxed\game_ready_objects`, and explicit transparent placeholders only for missing overlay/POI/route roles. Old overworld terrain/object/dock atlases are not runtime sources.
...
- The standalone `D:\atlas` approved material pack is promoted into runtime through `tools/world_assets/import_selected_world_assets.py`; rerun it after regenerating approved materials. Missing semantic material ideas or rejected cells are not failures and should stay out of runtime until approved.
...
- Normal Phaser overworld terrain is mask-rendered by `src/world/semantic/semanticMaskTerrainRenderer.ts`. It renders deep ocean, shallow water, beach, grass, sand, and ice/snow from semantic masks, using current selected PNG materials as repeating texture sources and small pixel-step boundary accents. The semantic grid remains the source of truth for movement, collision, encounters, harbors, roads, POIs, and island metadata. Styled road/river strokes, forests, mountain ranges, POIs, player, and UI remain separate layers above the mask background.
...
- Roads, beaches/coasts, shallow water, forests/jungle, mountains, volcanic accents, docks/bridges, and POIs are renderer/overlay concerns. Current overlay/POI/route sprites in `src/assets/world/current/` are placeholders until approved selected sprites exist. Road-river crossings are semantic bridge candidates and currently render by drawing the road stroke over the river; future bridge art should be separate overlay stamps, not road/river terrain atlases.
- Approved object overlays may replace forest, mountain, POI, resource, prop, and dock placeholders when the object clearly matches a semantic role. Cool approved but unmapped objects stay in the manifest as available only; do not randomly stamp them onto the map without a semantic generation role.
...
- Raw/debug/fallback road tile IDs are semantic compatibility only. Normal gameplay route endings are handled by the styled route overlay.
