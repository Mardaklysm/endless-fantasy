...
- Do not modify, crop, or heal `atlas_v3.png` to remove seams. Runtime seam cleanup is a cached map-level black seam repair pass only: it may replace near-black pixels inside expected tile seam/corner areas using a 3..6px shallow clean-source search and dual-side color mixing, but it must not blend terrain broadly or change non-black tile artwork.
...
- Raw water, lava, volcanoes, and blocked mountain tiles are not walkable. The current `atlas_v3_tile_world` pass does not generate roads, rivers, beaches, or bridges. Do not reintroduce boat/skyship terrain overrides for raw water unless the movement design is explicitly changed.
