...
- The active overworld keeps `src/assets/world/atlas_v3.png` and `src/assets/world/atlasV3.manifest.json` for semantic compatibility, raw-tile debug rendering, fallback tile drawing, and selected overlay helpers. The source is `D:\Projects\new_artwork\atlas_v3.jpeg`, imported once to PNG for runtime. Normal terrain presentation is the generated semantic mask texture, not the square atlas tile grid.
...
- Normal Phaser overworld terrain is rendered from the semantic fields by `src/world/semantic/semanticTerrainRenderer.ts` into a generated canvas texture. The renderer uses crisp full-cell terrain fills and deterministic pixel patterns, then draws narrow edge-only overlays for shallow-water, coast, beach/inland, and biome boundaries. The semantic grid remains the source of truth for movement, collision, encounters, harbors, roads, POIs, and island metadata. Roads, rivers, forests, mountains, POIs, player, and UI remain separate layers above the terrain background. The square `atlas_v3` terrain grid is debug/fallback only in normal world rendering.
...
- Do not reintroduce runtime map-level seam blending, same-tile seam softening, water seam blending, intersection/corner softening, or cached-pixel mutation for stamped `atlas_v3` tile maps. The accepted transition direction is semantic rendering with crisp fills and narrow code-drawn boundary overlays for shallow water, beaches, coastline foam/tint, and grass/sand/ice edges, not a giant transition tile atlas and not a whole-map blur.
