...
- The active overworld keeps `src/assets/world/atlas_v3.png` and `src/assets/world/atlasV3.manifest.json` for normal base terrain, semantic compatibility, raw-tile debug rendering, fallback tile drawing, and selected overlay helpers. The source is `D:\Projects\new_artwork\atlas_v3.jpeg`, imported once to PNG for runtime. Normal terrain presentation uses canonical atlas art plus a transparent semantic edge overlay; procedural full-cell semantic fills and random base tile variants are not the default gameplay renderer.
...
- Normal Phaser overworld terrain is atlas-backed. `src/world/semantic/semanticTerrainRenderer.ts` renders only a transparent canvas overlay with narrow edge accents for shallow-water, coast, beach/inland, and biome boundaries. The semantic grid remains the source of truth for movement, collision, encounters, harbors, roads, POIs, and island metadata. Roads, rivers, forests, mountains, POIs, player, and UI remain separate layers above the atlas terrain and edge overlay.
...
- Do not reintroduce runtime map-level seam blending, same-tile seam softening, water seam blending, intersection/corner softening, cached-pixel mutation, or random terrain-variant spam for stamped `atlas_v3` tile maps. The accepted transition direction is canonical atlas base terrain with narrow transparent code-drawn edge overlays for shallow water, beaches, coastline foam/tint, and grass/sand/ice boundaries. Do not replace the atlas with procedural full-cell fills, a giant transition tile atlas, or a whole-map blur.
