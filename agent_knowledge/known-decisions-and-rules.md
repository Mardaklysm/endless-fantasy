...
- Normal Phaser overworld terrain is mask-rendered by `src/world/semantic/semanticMaskTerrainRenderer.ts`. It renders deep ocean, shallow water, beach, grass, sand, and ice/snow from semantic masks, using current selected PNG materials as repeating texture sources and small pixel-step boundary accents. The semantic grid remains the source of truth for movement, collision, encounters, harbors, roads, POIs, and island metadata. Styled road strokes, directional material-filled river masks, forests, mountain ranges, POIs, player, and UI remain separate layers above the mask background.
...
- Roads and rivers are semantic graph/mask data, but normal rendering is split: `semanticRouteRenderer.ts` renders styled road strokes, while `semanticRiverTileRenderer.ts` renders river bodies from `world_current_terrain_freshwater` by drawing connected cardinal masks filled with the freshwater material. River graph strokes, source/mouth dots, and path diagnostics are F6/debug-only. Do not reintroduce smooth blue tube strokes, bezier river bodies, gradients, rounded caps, full-square freshwater stamps, or canvas-drawn river pools as normal gameplay river rendering.
- Approved terrain material fills support river rendering through the freshwater source sheet now. Future river improvements should add dedicated directional river edge/turn masks or better material fills to the asset mapping rather than returning to square atlas stamping or procedural blue tubes.
