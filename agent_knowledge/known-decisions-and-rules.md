...
- Island Kernel Lab is a standalone development utility under `tools/island-kernel-lab/` for validating generated Greenhaven island kernel art before integration. It is not part of Phaser runtime, does not replace current worldgen, and should not drive active gameplay.
- The official current Greenhaven island kernel format for that lab is a PNG at 1152x1152, organized as a 9x9 grid of 128x128 cells. Lab outputs under `tmp/island-kernel-lab/` are disposable preview artifacts.
...
- The active overworld direction uses semantic masks/fields first and visual rendering second: archipelago land masks, distance-driven shallow water/beaches, grass/sand/ice biome fields, separate elevation/ridge fields, and overlay objects/graphs for mountains, forests, rivers, roads, settlements, and POIs. Do not make a huge transition tileset or WFC the core world architecture.
- Normal Phaser overworld terrain is mask-rendered by `src/world/semantic/semanticMaskTerrainRenderer.ts`. It renders deep ocean, shallow water, beach, grass, sand, and ice/snow from semantic masks, using atlas terrain crops as repeating texture sources and small pixel-step boundary accents. The semantic grid remains the source of truth for movement, collision, encounters, harbors, roads, POIs, and island metadata. Forests, mountains, POIs, player, and UI remain separate layers above the mask background.
...
- Do not reintroduce runtime map-level seam blending, same-tile seam softening, water seam blending, intersection/corner softening, cached-pixel mutation, or random terrain-variant spam for stamped `atlas_v3` tile maps. The accepted transition direction is semantic mask rendering with atlas texture sources and crisp code-drawn mask boundaries. Do not replace this with a giant transition tile atlas, full island stamps, whole-map blur, or square tile terrain plus tiny edge strips as the normal view.
...
- The active overworld is an ocean archipelago with four profile-controlled major islands: Greenhaven, Coralreach, Frostmere, and Highspire, plus seeded minor islands. The generator tracks semantic island metadata, tile maps, towns, harbors, dungeons, landmarks, roads, rivers, shallow-water terrain, sea routes, reefs/details, and dock/bridge markers.
- Road and river graph strokes are debug overlays for now. Keep them out of normal gameplay rendering until they have a proper visual style; use F6 `roads` and F6 `rivers` to inspect the semantic data.
...
- Mountain placement is theme-capped and ridge-based. Greenhaven allows at most 2 normal mountains and no snow mountains; Coralreach allows at most 3 normal mountains and no snow mountains; Frostmere allows snow mountain clusters on ice/snow; Highspire has the strongest mountain identity, with snow mountains only on local ice/snow cells. Minor islands should avoid mountain spam and never get snow mountains unless a future explicit ice minor theme is added.
...
- Generated roads should connect to approach tiles outside POI footprints, not carve through landmark/town/dungeon centers. Keep road records synchronized with actual road tiles after any start-position or footprint cleanup. Road visuals must be derived from strict cardinal N/E/S/W masks; if the atlas lacks an orientation, rotate a known-good source tile at render/debug-preview time instead of trusting misleading tile ID names.
- The active `atlas_v3` cell named `road_vertical` visually contains a small side spur, so generated N+S roads intentionally render from the clean `road_horizontal` source rotated 90 degrees. Do not switch straight vertical roads back to `road_vertical` unless the atlas art itself is replaced or verified clean.
- Single-connector roads should render from the clean `road_dead_end_e` source tile, rotated to the needed direction. Non-city POI approaches should visually end with that cap; only town/city roads get a visual endpoint connector, and that connector must enter the bottom edge of the town footprint.
...
- The active overworld uses the `semantic_campaign_archipelago_world` generator plus separate POI/landmark/harbor marker rendering. It does not use classic-sheet landmark/object overlays, the 1885 extracted classic tile pool, or old 10x10 tile IDs.
