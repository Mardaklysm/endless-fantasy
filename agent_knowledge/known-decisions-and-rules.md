...
- The active overworld terrain uses the imported classic sheet `src/assets/world/tilesets/classic_world_tileset.cleaned.png` and manifest `src/assets/world/tilesets/classicWorldTileset.manifest.json`. `src/world/classicWorldTileCatalog.ts` curates high-confidence terrain/object/landmark entries from that large manifest for gameplay use.
- The older 10x10 generated overworld atlas `src/assets/world/world_atlas.normalized.png` is legacy/reference only and should not drive active gameplay unless a future task explicitly reactivates it.
...
- The active overworld uses the classic tileset-backed generated world plus classic-sheet landmark/object overlays. Generated terrain/marker drawing remains fallback only.
