...
- The active overworld art set is `src/assets/world/current/` plus `src/assets/world/current/world_asset_manifest.json`. It contains 37 approved individual 256x256 terrain fills imported from `D:\atlas\output\approved_materials`, 80 approved transparent 256x256 object sprites imported from `D:\new_items\output\approved_objects`, and explicit transparent placeholders only for missing overlay/POI/route roles. Old overworld terrain/object/dock atlases are not runtime sources.
