...
- Overlay collision policy is explicit: `visualOnly` and `softTerrain` overlays are walkable, `hardBlock` overlay-policy cells block movement, and `poiBlock` marks interaction/building cells for the current POI system. Normal tree/forest overlays are `softTerrain` and passable for now. Mountain blocking comes from the semantic mountain mask and its `hardBlock` policy cells only; mountain sprites are `visualOnly` and must not use sprite bounds for collision. Unbridged inland river cells are `hardBlock`; road-river crossing candidates remain walkable.
...
- Palm groves, pine groves, and normal broadleaf groves should be clustered/zone-based and spatially separated, not tile-by-tile checkerboards. Runtime forest overlays choose one object family per connected semantic grove based on island theme/biome; avoid mixing palms and broadleaf trees inside the same grove unless a future explicit transition biome is added.
...
- Overworld POI footprints are interaction/collision affordances, not visual size. Towns, harbors, and final landmarks may use 3x3 footprints, but most non-city POIs should remain 1x1 and render compactly. POI asset selection is island/theme-aware: ice regions use snow/ice variants when present, Coralreach uses sand/coastal variants, Highspire uses volcanic/dark/stone variants, and unrelated swamp POIs must not appear as generic fallbacks outside swamp-themed regions.
- POI placement must validate land before placement. Normal POIs must not spawn on shallow water, rivers, lakes, mountains, or invalid coast cells, and settlement/final footprints must validate their whole footprint. Harbor anchors must be valid beach land adjacent to shallow water, with any water-facing visual handled separately.
- POI aprons are subtle renderer overlays around/under POIs, not base terrain stamping. They must skip rivers, roads, lakes, water, and inappropriate beach cells, with theme-aware dirt/sand/snow/ash colors.
