...
- Roads and rivers are semantic paths rendered by `semanticRouteRenderer.ts` as styled procedural overlays in normal gameplay. They are not base terrain tiles, do not need giant road/river atlases, and should not show debug graph dots/nodes/source markers outside F6 diagnostics. The current river visual is a muted banked procedural brush with curved strokes, shadowed banks, source/mouth pools, and deterministic flecks; future river art should improve that renderer/brush path rather than baking rivers into terrain or object sprites.
