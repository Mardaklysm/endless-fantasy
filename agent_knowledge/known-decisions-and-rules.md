...
- Normal Phaser overworld terrain is mask-rendered by `src/world/semantic/semanticMaskTerrainRenderer.ts`. It renders deep ocean, shallow water, freshwater rivers/lakes, packed-dirt roads, beach, grass, sand, ice/snow, and mountain massifs from semantic masks, using manifest-selected PNG materials as repeating texture sources and small pixel-step boundary accents. Water, beach/sand, freshwater, and snow/ice should keep using `terrain_v1` until a visually approved replacement beats the restored atlas v3 look. The semantic grid remains the source of truth for movement, collision, encounters, harbors, roads, mountain blocking, POIs, and island metadata. Forests, decorative ridge sprites, POIs, bridges/docks, player, clouds, and UI remain separate layers above the mask background.
...
- Overlay collision policy is explicit: `visualOnly` and `softTerrain` overlays are walkable, `hardBlock` overlay-policy cells block movement, and `poiBlock` marks interaction/building cells for the current POI system. Normal tree/forest overlays are `softTerrain` and passable for now. Mountain blocking comes from the semantic mountain mask and its `hardBlock` policy cells only; decorative mountain/ridge sprites are `visualOnly` and must not use sprite bounds for collision. Unbridged inland river cells are `hardBlock`; road-river crossing candidates remain walkable.
- Mountain placement is semantic-mask-first, ridge/elevation-derived, and massif-based. The generator uses high ridge/elevation candidates for preliminary POI guidance, then reselects final connected mountain masks after POIs, rivers, and roads are known so mountain cells stay away from towns, ports, roads, lakes, rivers, and important POI footprints. `mountainCap` is a soft per-island massif cell budget used by generation planning; smoothing may slightly exceed it when preserving organic connected landforms. Cleanup removes components below island minimum size and must leave zero singleton mountain components. Runtime visuals render every accepted hard-blocked mountain cell as opaque mountain/massif terrain through `semanticMaskTerrainRenderer.ts`, then place a small number of interior/centerline premium mountain/ridge overlays per accepted semantic range. Collision remains on the accepted semantic mask cells; visual-only ridge sprites must never add extra collision. Greenhaven/Coralreach get restrained normal massifs and no snow mountains; Frostmere supports large snow mountain fields on ice/snow; Highspire has the strongest mountain identity, with large ridge/massif regions. Minor islands should avoid mountain spam and never get snow mountains unless a future explicit ice minor theme is added.
