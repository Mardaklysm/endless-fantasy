...
- Runtime assets live under `src/assets/` and are loaded by Phaser through `src/assets/assetPaths.ts`, `src/assets/textureKeys.ts`, and `src/scene/sceneLifecycle.ts`. Root `assets/` and `assets_v2/` are retired.
- Fighter/priest/wizard class sprite sheets are imported from `D:\Tools\rembg\bg_output`; source copies live under `src/assets/source/art_import/source_sheets/class_sprites/`, runtime sheets live under `src/assets/heroes/`, and frame/anchor metadata lives in `src/data/characterSprites.ts`.
...
- The active dungeon/city tile layer uses individual PNGs under `src/assets/world/dungeon_tiles/` plus `src/assets/world/dungeonTiles.manifest.json`. Runtime dungeon atlas cropping is retired.
...
- Normal Phaser overworld terrain is mask-rendered by `src/world/semantic/semanticMaskTerrainRenderer.ts`. It renders deep ocean, shallow water, freshwater rivers/lakes, packed-dirt roads, beach, grass, sand, and ice/snow from semantic masks, using manifest-selected PNG materials as repeating texture sources and small pixel-step boundary accents. Water, beach/sand, freshwater, and snow/ice should keep using `terrain_v1` until a visually approved replacement beats the restored atlas v3 look. The semantic grid remains the source of truth for movement, collision, encounters, harbors, roads, mountain blocking, POIs, and island metadata. Forests, one visual-only mountain sprite per semantic mountain cell, POIs, bridges/docks, player, clouds, and UI remain separate layers above the mask background.
...
- Overlay collision policy is explicit: `visualOnly` overlays are walkable, `softTerrain` forest overlays are walkable for now, `hardBlock` mountain/river cells block movement, and `poiBlock` POI body cells block movement while allowing edge activation. Tree, forest, mountain, and POI sprite bounds must not add collision. Mountain blocking comes from the semantic mountain mask and its `hardBlock` policy cells only; mountain sprites are `visualOnly`. Unbridged inland river cells are `hardBlock`; validated road-river crossing cells are marked in `riverCrossingMap`, remain walkable, and are the only default source for visible bridge/ford overlays.
- Mountain placement is semantic-mask-first, ridge/elevation-derived, and component-based. The generator uses high ridge/elevation candidates for preliminary POI guidance, then reselects final connected mountain masks after POIs, rivers, and roads are known so mountain cells stay away from towns, ports, roads, lakes, rivers, and important POI footprints. `mountainCap` is a soft per-island mountain cell budget used by generation planning; smoothing may slightly exceed it when preserving organic connected landforms. Cleanup removes components below island minimum size and must leave zero singleton mountain components. Runtime visuals place one normal unscaled visual-only mountain sprite on every accepted hard-blocked semantic mountain cell; normal gameplay must not draw gray mountain terrain rectangles or sparse scaled ridge anchors. Collision remains on the accepted semantic mask cells. Greenhaven/Coralreach get restrained normal mountains and no snow mountains; Frostmere supports snow mountain fields on ice/snow; Highspire has the strongest mountain identity. Minor islands should avoid mountain spam and never get snow mountains unless a future explicit ice minor theme is added.
...
- Palm groves, pine groves, and normal broadleaf groves should be clustered/zone-based and spatially separated, not isolated random single trees. Runtime forest overlays choose one object family per connected semantic grove based on island theme/biome and place one normal unscaled tree sprite on every semantic forest cell; forest cells keep a one-cell clearance from ocean/coast water, lakes, and rivers so tree sprites do not overhang water. Avoid mixing palms and broadleaf trees inside the same grove unless a future explicit transition biome is added.
...
- Overworld POI footprints are interaction/collision affordances and the current visual footprint. Settlements/final landmarks use 3x3 bodies; harbors, dungeons, gates, shrines, caves, ruins, towers, resources, and other landmarks use at least 2x2 bodies. POI asset selection is island/theme-aware: ice regions use snow/ice variants when present, Coralreach uses sand/coastal variants, Highspire uses volcanic/dark/stone variants, and unrelated swamp POIs must not appear as generic fallbacks outside swamp-themed regions.
- POI placement must validate land before placement. Normal POIs must not spawn on shallow water, rivers, lakes, mountains, roads, forests, or invalid coast cells, and their whole footprint must validate. Harbor footprints must include valid land cells with a shallow-water edge for docks/travel.
- Do not draw runtime POI apron/footprint tint squares. If a future plaza/clearing returns, it must use proper opaque terrain/detail art and still skip rivers, roads, lakes, water, and inappropriate beach cells. Debug/lab views may color POI footprints.
...
- Title presentation uses `src/assets/title/title_screen.png` as a full-screen image, drawn with aspect-preserving contain fit over a black background. Keep the title menu as live text ordered `Continue`, then `New Game`, and do not reintroduce title-screen control/help subtitles.
...
- Overworld locations use larger blocked 2x2/3x3 visual/body footprints with edge activation so landmarks read as important instead of one-tile icons.
...
- Dungeon tile PNGs already have the old source-grid seam inset baked in. Do not reintroduce runtime atlas cropping.
