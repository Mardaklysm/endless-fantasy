...
- The overworld terrain atlas is imported from the final PNG `C:\Users\Marku\Downloads\master_overworld_tileset_atlas_10x10.png` by `tools/art_import/import_world_atlas.mjs`. The source is copied exactly to `src/assets/world/world_atlas.normalized.png`, a provenance copy is kept in `src/assets/world/source/`, and the 100-tile 10x10 manifest lives in `src/data/worldTiles.ts`.
- The complex classic overworld sheet `C:\Users\Marku\Downloads\57105.png` is imported only as an inactive candidate pack by `tools/art_import/import_classic_world_tileset.mjs`. Its source PNG has no meaningful alpha despite RGBA color type, so exact `#00B100` chroma-key removal creates `src/assets/world/tilesets/classic_world_tileset.cleaned.png`; the manifest is `src/assets/world/tilesets/classicWorldTileset.manifest.json`.
...
- Do not wire the imported `classic_world_tileset` candidate pack into runtime until rights/provenance and the generated low-confidence manifest entries have been reviewed.
