...
- Overworld generation has two supported modes. `classicIsland` is the current default for testing; `generic10x10` preserves the existing 10x10 atlas-based generator for later use.
- `classicIsland` uses the imported classic sheet `src/assets/world/tilesets/classic_world_tileset.cleaned.png`, manifest `src/assets/world/tilesets/classicWorldTileset.manifest.json`, and curated region 7+10 entries from `src/world/classicGrasslandRegionCatalog.ts`. It must not consume the full 1885 extracted tile pool directly.
- `generic10x10` uses `src/assets/world/world_atlas.normalized.png` through `src/world/genericAtlasWorldCatalog.ts` and `src/world/genericAtlasWorldGenerator.ts`.
- New overworlds are seeded and generated by the dispatcher in `src/world/worldGenerator.ts`. New Game creates a fresh seed using the active mode; Save/Load persists and rebuilds from `worldSeed`.
...
- Classic island mode uses classic-sheet landmark/object overlays and a small special-map POI set. It does not visually preserve or force the old ten progression POIs. Generic mode still owns the old broader progression layout.
...
- Current generic progression is Root -> Flame/Tide/Gale -> Starfall Gate -> Eclipse Spire. Classic island mode intentionally uses a smaller POI set for map composition and maps those POIs to existing interiors only as a compatibility adapter.
