...
| Harbor docks | `drawPierDockTile` | `src/assets/world/current/routes/*.png` | Low; current route sprites are placeholders until approved dock/bridge art exists |
| World object overlays | `drawWorldObjectCell`, `drawLocationIcon`, `drawWorldOverlays` | `src/assets/world/current/objects/*.png`, remaining placeholders, and manifest mappings in `src/data/worldCurrentAssets.ts` | Active path; generated POIs carry object IDs and ocean detail overlays are seed-derived from reef positions. Approved world-object PNGs replace many placeholders; missing roles remain explicit placeholders |
