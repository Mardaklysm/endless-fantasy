...
Current note: the Phaser scene already has a `preload()` image loader. Normal overworld terrain uses `src/world/semantic/semanticMaskTerrainRenderer.ts` to generate a crisp semantic mask background. The renderer fills deep ocean, shallow water, freshwater, beach, grass, sand, and snow/ice masks from individual selected PNG materials described by `src/assets/world/current/world_asset_manifest.json`. Water, beach/sand, freshwater, and snow/ice currently use restored atlas v3 cell extracts in `src/assets/world/current/terrain_v1/`, while the newer approved material pack remains in `src/assets/world/current/terrain/` as v2/future candidates. Individual root `assets/tiles/world/*` files are fallback references. Old overworld terrain/object/dock atlas renderers are removed from normal runtime and must not be reintroduced as equal paths. Current forest, mountain, POI, resource, prop, dock, bridge, and settlement/city/harbor/castle POI roles now resolve premium-first from `src/assets/world/current/objects_premium/`, imported from `D:\Tools\rembg\bg_output` by `tools/world_assets/import_premium_world_objects.py`; the curated `src/assets/world/current/objects/` folder remains backup/fallback. Compact villages, towns, city districts, harbors, castle towns, forts, monasteries, and academies are valid POI sprites when they look good. Overworld cloud flavor resolves from `src/assets/world/current/clouds/cloud_manifest.json`: reuse the five base cloud masks for all islands, apply theme tint/alpha/speed in runtime, and only fill `themeCloudPools` later if we add true biome-specific cloud art. Remaining missing roles stay explicit current-object or generated fallbacks. Dungeon/city tiles still use the opaque `src/assets/world/dungeon_atlas.png` sheet plus `src/data/dungeonTiles.ts`, cropped with `DUNGEON_ATLAS_SOURCE_INSET = 3`. Runtime must not use full-map blur, huge coastline tile libraries, rendered PNG packages as gameplay truth, random terrain variant spam, or fake road/river graph strokes in normal gameplay.
...
|---|---|---|---|
| World terrain | `createSemanticMaskTerrainTexture`, `drawWorldTile` raw/debug fallback | `src/assets/world/current/world_asset_manifest.json` plus `current/terrain/*.png` and `current/terrain_v1/*.png` | Active normal path is mask-rendered from semantic fields with selected individual material PNGs. Restored v1 atlas-cell extracts are active for water/sand/snow; F6 `rawTiles` uses manifest-selected material texture keys, not atlas cell rendering |
| Harbor docks | `drawPierDockTile` | `src/assets/world/current/objects_premium/*.png`, then `src/assets/world/current/objects/*.png` | Low; premium dock/bridge stamps are preferred, with curated backup objects or procedural route strokes as fallback |
| World object overlays | `drawWorldObjectCell`, `drawLocationIcon`, `drawWorldOverlays` | `src/assets/world/current/objects_premium/*.png`, then `src/assets/world/current/objects/*.png`, plus manifest mappings in `src/data/worldCurrentAssets.ts` | Active path; generated POIs carry object IDs and ocean detail overlays are seed-derived from reef positions. Premium world-object PNGs replace matching roles first; backup approved objects remain available for gaps |
| Overworld cloud overlay | `OverworldCloudOverlay` | `src/assets/world/current/clouds/cloud_manifest.json` plus `current/clouds/cloud_base_*.png` | Low; visual-only top-band overlay on world mode. Base cloud masks are reused with theme tint/alpha/speed, and future non-empty theme pools can override the base masks |
...
2. Replace `drawWorldTile` with selected-material rendering. Done for `src/assets/world/current`; normal overworld terrain now uses semantic texture generation from individual PNG fills while raw square tiles remain debug/fallback.
3. Add world object overlay rendering for generated dungeons, landmarks, harbors, and ocean detail POIs. Current-folder approved object sprites are wired for many roles; keep placeholders only for roles without a clean approved replacement.
