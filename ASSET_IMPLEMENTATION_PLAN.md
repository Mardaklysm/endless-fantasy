...
| World object overlays | `drawWorldObjectCell`, `drawLocationIcon`, `drawWorldOverlays` | `src/assets/world/current/overlays/*.png`, `current/pois/*.png`, and manifest mappings in `src/data/worldCurrentAssets.ts` | Active path; generated POIs carry object IDs and ocean detail overlays are seed-derived from reef positions. Placeholders are explicit and should be replaced with approved selected sprites |
...
3. Add world object overlay rendering for generated dungeons, landmarks, harbors, and ocean detail POIs. Current-folder placeholders are wired; replace them with approved selected sprites when available.
