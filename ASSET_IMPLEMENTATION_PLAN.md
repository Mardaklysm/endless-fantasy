...
Current note: the Phaser scene already has a `preload()` image loader. Normal overworld terrain uses the `atlas_v3` texture from `src/assets/world/atlas_v3.png`, imported manifest `src/assets/world/atlasV3.manifest.json`, and tile metadata in `src/data/worldTiles.ts` as the atlas-backed base terrain. `src/world/worldGenerator.ts` maps semantic base terrain to one canonical tile per role by default: `deep_water`, `shallow_water`, `beach_sand`, `medium_grass`, `bright_sand`, and `clean_snow`. `src/world/semantic/semanticTerrainRenderer.ts` generates only a transparent edge-overlay texture for shallow-water, coastline, wet-sand, beach/inland, and biome boundary accents. Individual root `assets/tiles/world/*` files are fallback/legacy references. Normal/raw/fallback atlas draws use `ATLAS_V3_SOURCE_INSET = 3` to crop dirty source-cell edges while drawing each valid tile into the full destination tile. Harbor dock overlays use `src/assets/world/pier_atlas.png`. Generated POI and ocean-detail object overlays use the transparent `src/assets/world/world_objects.png` sheet plus `src/data/worldObjects.ts`. Dungeon/city tiles use the opaque `src/assets/world/dungeon_atlas.png` sheet plus `src/data/dungeonTiles.ts`, cropped with `DUNGEON_ATLAS_SOURCE_INSET = 3`. Runtime must not run map-level seam blending, mutate cached terrain pixels with neighboring terrain colors, or replace atlas terrain with procedural full-cell fills.
...
|---|---|---|---|
| World terrain | `SEMANTIC_BASE_TILE_PALETTE`, `drawWorldTile` / atlas terrain cache plus `createSemanticEdgeOverlayTexture` | `src/assets/world/atlas_v3.png` + `src/assets/world/atlasV3.manifest.json` + `src/data/worldTiles.ts`; transparent semantic edge overlay from `src/world/semantic/semanticTerrainRenderer.ts` | Active normal path uses exact 8x8 atlas cells as the base terrain with canonical tile mapping; semantic rendering only adds transparent edge accents and can be toggled off with F6 `edgesOff` |
