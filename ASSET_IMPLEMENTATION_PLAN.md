...
|---|---|---|---|
| World terrain | `createSemanticMaskTerrainTexture`, `drawWorldTile` raw/debug fallback | `src/assets/world/current/world_asset_manifest.json` plus `current/terrain/*.png` | Active normal path is mask-rendered from semantic fields with selected individual material PNGs. F6 `rawTiles` uses current material texture keys, not atlas cells |
